const DATA_URL = 'neet_pg_recall_quiz_data.json';
...
let current = 0;
let imageOnly = false;
const REPORT_EXAM = 'NEET-PG';
const REPORT_ENDPOINT = '/api/report';
let DATA = [];
const attempts = new Map();
const $ = s => document.querySelector(s);
const tabs = [...document.querySelectorAll('.year-tab')];
const ITEM_KEYS = new Set(['year', 'number', 'originalNumber', 'question', 'options', 'answerIndex', 'source', 'status', 'note', 'imageHint', 'images', 'imageRef', 'subject', 'subjectTags', 'sourceSubject', 'explanation', 'references', 'uploadedScreenshotQuestion', 'session']);
const IMAGE_KEYS = new Set(['src', 'caption', 'credit', 'source', 'width', 'height']);

function dataError(message) { throw new TypeError(`Invalid NEET-PG recall data: ${message}`); }
...
function optionalSafeInteger(value, field) {
  ...
  if (!Number.isSafeInteger(value) || value < 1 || value > 10000) dataError(field);
  return value;
}
function optionalLegacyIdentifier(value, field) {
  if (value === undefined || value === null) return undefined;
  if (Number.isSafeInteger(value) && value >= 1 && value <= 10000) return value;
  if (typeof value === 'string' && /^[A-Za-z][A-Za-z0-9 &()./-]{0,99}$/.test(value)) return value;
  dataError(field);
}
function normalizeQuestion(raw, index) {
  ...
  const imageRef = raw.imageRef === undefined ? '' : normalizeAssetPath(raw.imageRef, `${field}.imageRef`);
  const normalized = {
    year: raw.year,
    number: raw.number,
    ...
    source: readString(raw.source, `${field}.source`, 500),
    session: optionalString(raw.session, `${field}.session`, 200),
    ...
    status: optionalString(raw.status, `${field}.status`, 100),
    note: optionalString(raw.note, `${field}.note`, 5000),
    imageHint: optionalString(raw.imageHint, `${field}.imageHint`, 1000),
    sourceSubject: optionalString(raw.sourceSubject, `${field}.sourceSubject`, 150),
    images: normalizeImages(raw.images, `${field}.images`),
    references: normalizeReferences(raw.references, `${field}.references`),
    imageRef
  };
  const originalNumber = optionalLegacyIdentifier(raw.originalNumber, `${field}.originalNumber`);
  const uploadedScreenshotQuestion = optionalSafeInteger(raw.uploadedScreenshotQuestion, `${field}.uploadedScreenshotQuestion`);
  if (originalNumber !== undefined) normalized.originalNumber = originalNumber;
  if (uploadedScreenshotQuestion !== undefined) normalized.uploadedScreenshotQuestion = uploadedScreenshotQuestion;
  return Object.freeze(normalized);
}
async function loadQuestionData() {
  ...
  for (const item of normalized) {
    const id = [item.year, item.session || item.source, item.number].join('::');
...
function isImageBased(item) {
  ...
  const hasAssetRef = typeof item.imageRef === 'string' && item.imageRef.startsWith('assets/');
...
function openReport(item, practiceNumber, practiceTotal) {
  ...
  const summary = `${REPORT_EXAM} ${item.year} Q${item.number}`;
  form.reset();
  ...
  form.elements.session.value = item.session || item.source;
...
function filtered() {
  ...
  const items = DATA.filter(item => String(item.year) === String(year)).filter(item => {
...
function explanationHtml(item) {
  ...
  const sources = (item.references || []).filter(ref => ref && ref.title && /^https:\/\//.test(ref.url || ''));
  const sourceHtml = sources.length ? '<div class="explanation-sources"><strong>Current guidance:</strong> ' + sources.map(ref => '<a href="' + escapeHtml(ref.url) + '" target="_blank" rel="noopener noreferrer">' + escapeHtml(ref.title) + '</a>').join(' · ') + '</div>' : '';
  return '<section class="explanation-card"><h3>Explanation</h3>' + paragraphs + sourceHtml + '</section>';
}
function renderQuiz() {
  const items = filtered();
  ...
  let explanation = '';
  if (att && canCheck) {
    feedback = `<div class="feedback ${att.correct ? 'ok' : 'bad'}">${att.correct ? 'Correct.' : 'Wrong.'} Correct answer: ${String.fromCharCode(65 + item.answerIndex)}. ${escapeHtml(item.options[item.answerIndex])}</div>`;
    explanation = explanationHtml(item);
  }
  const remaining = Math.max(items.length - current - 1, 0);
  const progress = items.length ? Math.round(((current + 1) / items.length) * 100) : 0;
  const subjectPill = `<span class="pill topic">${escapeHtml((item.subjectTags && item.subjectTags[0]) || item.subject || 'NEET-PG')}</span>`;
...
tabs.forEach(t => t.onclick = () => { year = t.dataset.year; current = 0; renderList(); renderQuiz(); });
...
async function initialize() {
  try {
    ...
    console.error('NEET-PG recall questions could not be loaded safely.', error);
