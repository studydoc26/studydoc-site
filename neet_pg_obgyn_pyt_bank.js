...
function renderQuiz() {
  ...
  const imageNote = '';
  const images = imageList(item).length ? `<div class="qimages">${imageList(item).map((image, i) => `<figure><img src="${escapeHtml(imageSrc(image))}" alt="ObGyn PYT image ${i + 1} for question ${current + 1}" loading="lazy"></figure>`).join('')}</div>` : '';
